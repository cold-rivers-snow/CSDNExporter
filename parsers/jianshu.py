"""
JianShu Parser
"""
import re
from typing import List
from .base import SiteParser, ArticleMetadata
from bs4 import NavigableString


class JianShuParser(SiteParser):
    """Parser for JianShu (简书) blogs"""
    
    SITE_NAME = "简书"
    SITE_DOMAINS = ['jianshu.com', 'jianshu.io']
    
    SELECTORS = {
        'title': ['h1', '.title', '.article-title'],
        'content': ['div.article-content', 'div.note-text', 'div._1-OS'],
        'publish_time': ['span.time', 'time'],
        'author': ['a.author-name', 'span.nickname', 'a.name'],
        'category': ['div.category', 'a.category-link'],
        'tags': ['div.tags', 'a.tag'],
    }
    
    def get_metadata(self) -> ArticleMetadata:
        soup = self.soup
        
        # Title
        title = ""
        title_tag = soup.find('h1')
        if not title_tag:
            title_tag = soup.find('div', {'class': lambda x: x and 'title' in x if x else False})
        if title_tag:
            title = title_tag.get_text(strip=True)
        
        # Publish time
        publish_time = ""
        time_tag = soup.find('span', {'class': 'time'})
        if not time_tag:
            time_tag = soup.find('time')
        if time_tag:
            publish_time = time_tag.get_text(strip=True)
        
        # Author
        author = ""
        author_tag = soup.find('span', {'class': 'nickname'})
        if not author_tag:
            author_tag = soup.find('a', {'class': 'name'})
        if author_tag:
            author = author_tag.get_text(strip=True)
        
        # Category
        category = ""
        cat_tag = soup.find('a', {'class': 'category-link'})
        if cat_tag:
            category = cat_tag.get_text(strip=True)
        
        # Tags
        tags = []
        tag_list = soup.find_all('a', {'class': lambda x: x and 'tag' in x if x else False})
        tags = [a.get_text(strip=True) for a in tag_list if a.get_text(strip=True)]
        
        return ArticleMetadata(
            title=title,
            publish_time=publish_time,
            author=author,
            tags=tags,
            category=category,
            url=self.url
        )
    
    def get_content(self) -> str:
        content = ""
        
        # Main content areas
        content_tags = self.soup.find_all('div', {'class': 'article-content'})
        if not content_tags:
            content_tags = self.soup.find_all('div', {'class': 'note-text'})
        if not content_tags:
            content_tags = self.soup.find_all('div', {'class': '_1-OS'})
        
        for tag in content_tags:
            content += str(tag)
        
        return content
