...
class LofterParser(SiteParser):
    """Parser for Lofter (乐乎) blogs"""
    
    SITE_NAME = "Lofter"
    SITE_DOMAINS = ['lofter.com', 'lauxiang.net']
    
    SELECTORS = {
        'title': ['h1.title', '.posttitle', 'h1'],
        'content': ['div.post-content', 'div.text', 'div.content'],
        'publish_time': ['span.date', 'time', '.publish-time'],
        'author': ['a.author', '.nickname', '.blog-author'],
    ...
    def get_content(self) -> str:
        ...
        if not content_tags:
            content_tags = self.soup.find_all('div', {'class': 'text'})
